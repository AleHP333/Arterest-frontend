export const addToFav = (
  ...
  if (favs === null || !favs.length) {
    localStorage.setItem(
      "favList",
      JSON.stringify([{ userName, userImage, title, img, _id, price }])
    );
    ...
    if (found) {
      ...
      localStorage.setItem(
        ...
        JSON.stringify([
          ...favs,
          { userName, userImage, title, img, _id, price },
        ])
      );
      setFavProducts(JSON.parse(localStorage.getItem("favList")));
    }
...
export const addToCart = (
  ...
  if (cart === null || !cart.length) {
    localStorage.setItem(
      "cartList",
      JSON.stringify([{ stock, userName, title, img, _id, price, quantity: 1 }])
    );
  } else {
    ...
    if (found) {
      ...
      localStorage.setItem(
        ...
        JSON.stringify([
          ...cart,
          { stock, userName, title, img, _id, price, quantity: 1 },
        ])
      );
    }
  }
